...
void eval(char *cmdline)
{
  ...
  strcpy(sbuf, cmdline);
  //make argv vector
  background = parseline(sbuf, argv, &pipec);
  pid_t pid[pipec];
  int fileds[pipec][2];
  ...
  if(!builtin_cmd(&(argv[i]))){
    ...
    if (pipe(&fileds[i][0])<0) perror("cannot install pipe");
  ...
  /*
    ...
    if(!background){
      ...
      printf("[%d] (%d) %s", pid2jid(pid[i]),pid[i], *argv[i]);
    }
    }
  }
  return;
...
void do_bgfg(char *(*argv)[MAXARGS] )
{
  ...
  if(!strcmp(*argv[0], "bg")) flag = 0;//argv[0][1]
  else flag = 1;
