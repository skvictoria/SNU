...
for alpha in alphas:
    ...
    synapse_1 = 2*np.random.random((5,5)) - 1
    synapse_2 = 2*np.random.random((5,1)) - 1
    ...
    for j in range(60000):
        ...
        if (j% 10000) == 0:
            ...
            print(str(j))
            print("iterations:")
            print(str(np.mean(np.abs(layer_2_error))))
    ...
    for ii in range(X_test.shape[0]):
        ...
        l3 = sigmoid(np.dot(synapse_2.T,l2))
        
        sums = sums + np.abs(l3-y_test[ii])
    print("Test Error : ")
    ...
    arctime=np.array([[55007, 55000], [50200 ,112400],[56700 ,161000],[55500, 57621],[57200 ,100000]])
    arctime = arctime //40000
        
    for ii in arctime:
        l0 = np.reshape(ii, (2,-1))   # l0 = 2x1 one by one
        l1 = sigmoid(np.dot(synapse_0.T,l0))  # l1 = 4x1
        l2 = sigmoid(np.dot(synapse_1.T,l1))
        l3 = sigmoid(np.dot(synapse_2.T,l2))
        print(l2)
